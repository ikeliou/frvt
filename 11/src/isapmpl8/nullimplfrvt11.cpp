...
NullImplFRVT11::initialize(const std::string &configDir)
{
    ...
    init(this->configDir+"/model/face.param", this->configDir+"/model/face.bin");
        ...
        std::vector<EyePair> &eyeCoordinates)
{
		...
		if (boxes.size()!=0) {
			...
			for (size_t j=0; j<boxes.size(); ++j) {
                ...
                for (unsigned k=0; k<5; ++k) {
                    crop.pts.push_back(cv::Point2f(box.point[k]._x/scale,box.point[k]._y/scale));
                }
#ifdef debug
	...
	if (crops.size()>0) {
        ...
        detectFace(aligned_mat, boxes);
        std::cout << "t1" <<std::endl;
        if (boxes.size()!=0) {
            std::cout << "t1" <<std::endl;
            cv::Rect rect(boxes[0].x1, boxes[0].y1, boxes[0].x2 - boxes[0].x1, boxes[0].y2 - boxes[0].y1);
            for (auto box:boxes) {
                cv::Rect rt(box.x1, box.y1, box.x2 - box.x1, box.y2 - box.y1);
                if (rt.area()>rect.area()) {
                    rect=rt;
                }
            }
